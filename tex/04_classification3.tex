...
\uncover<4->{
and we add one additional constraint for $x_i$ placed on the margins: $$\boxed{y_i (\vec{x}_i \cdot \vec{w} + b) -1 = 0}$$
}
\end{frame}
...
\textbf{One solution: compute the width with a unit (light blue) vector and project the purple vector on that unit vector}
\begin{center}
...
$$\max \frac{1}{\lVert \vec{w} \rVert} = \min \lVert \vec{w} \rVert \leadsto \min \frac{1}{2} \lVert \vec{w} \rVert^2$$}
...
\end{equation}
\begin{itemize}
\uncover<2->{\item W is only dependent on $\alpha$}
\uncover<3->{
...
\item fast test time execution (few $\alpha_i > 0$, linear: $\vec{w}$ can be pre-computed, non-linear: no pre-computation of $\vec{w}$ guaranteed [e.g. Gaussian kernel] but still fast due to inner products)
\end{itemize}
\uncover<2->{Disadvantages
\begin{itemize}
\item data samples have to be stored (space complexity not negligible)
