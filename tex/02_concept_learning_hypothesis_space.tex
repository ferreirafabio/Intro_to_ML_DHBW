...
\date{}
\graphicspath{{figures/concept_learning/}}
...
\begin{itemize}
\item Learning often involves inducing general functions from specific (training) examples %(inductive learning)
...
\item \uncover<2->{"x satisfies h" if $h(x) = 1$, $\exists x \in X$
...
Problems that face the \textit{curse of dimensionality} in inductive learning typically refer to the phenomenon that, when increasing the model dimensionality\footnote{e.g. attributes of concepts <x1, x2...>, weights/learned parameters in parameterized models}, the available data becomes sparse\\
$\Rightarrow$ the demand for additional data often grows exponentially with the dimensionality
\end{frame}

\begin{frame}{In Practice: Structural Risk Minimization}

\begin{itemize}
\item computing the VC dimension is often intractable or impossible
\item e.g. when need to prove that it's impossible to find any set that can be shattered
\end{itemize}

\uncover<4->{\begin{block}{Structural Risk Minimization (SRM)}
  \begin{enumerate}
  \item choose a class of functions (e.g. polynomials of degree n)
  \item divide the class of functions into subsets: $H_1 \subset H_2 \subset ... \subset H_n$ (e.g. polynomials of increasing degree)
  \item perform parameter selection (optimize)
  \item select the model with lowest (empirical) error (and implicitly lowest VC dim) 
  \end{enumerate}
\end{block}}
\uncover<5->{for example SVM performs SRM}
